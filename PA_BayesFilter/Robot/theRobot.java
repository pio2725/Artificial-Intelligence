...
public class theRobot extends JFrame {
    ...
    double getSensor(String sonars, int curr_x, int curr_y) {
        int match = 0;
        for (int y = 1; y < mundo.height - 1; y++) {
            for (int x = 1; x < mundo.width - 1; x++) {
                if (x == curr_x && y == curr_y) {
                        if (mundo.grid[x][y - 1] == Integer.parseInt(Character.toString(sonars.charAt(0)))) {
                            ++match;
                        }
                        if (mundo.grid[x][y + 1] == Integer.parseInt(Character.toString(sonars.charAt(1)))) {
                            ++match;
                        }
                        if (mundo.grid[x + 1][y] == Integer.parseInt(Character.toString(sonars.charAt(2)))) {
                            ++match;
                        }
                        if (mundo.grid[x - 1][y] == Integer.parseInt(Character.toString(sonars.charAt(3)))) {
                            ++match;
                        }
                    break;
                }
        ...
        double result = Math.pow(sensorAccuracy, match) * Math.pow((1 - sensorAccuracy), (4 - match));
        return result;
    }

    double getTransition(int action, int curr) {
    ...
    void updateProbabilities(int action, String sonars) {
        ...
        double sum = 0.0;
        double[][] newProbs = new double[mundo.height][mundo.width];
        for (int y = 0; y < mundo.height; y++) {
            for (int x = 0; x < mundo.width; x++) {
                newProbs[x][y] = 0.0;
            }
        ...
        for (int y = 1; y < mundo.height - 1; y++) {
            for (int x = 1; x < mundo.width - 1; x++) {
                ...
                if (probs[x][y] != 0.0) {
                    newProbs[x][y] += getTransition(action, STAY) * probs[x][y];
                }
                if (probs[x][y-1] != 0.0) {
                    newProbs[x][y - 1] += getTransition(action, NORTH) * probs[x][y];
                }
                if (probs[x][y+1] != 0.0) {
                    newProbs[x][y + 1] += getTransition(action, SOUTH) * probs[x][y];
                }
                if (probs[x-1][y] != 0.0) {
                    newProbs[x - 1][y] += getTransition(action, WEST) * probs[x][y];
                }
                if (probs[x+1][y] != 0.0) {
                    newProbs[x + 1][y] += getTransition(action, EAST) * probs[x][y];
                }
            }
        }
        ...
        for (int y = 1; y < mundo.height - 1; y++) {
            for (int x = 1; x < mundo.width - 1; x++) {
                    newProbs[x][y] *= getSensor(sonars, x, y);
                    sum += newProbs[x][y];
            }
        }
    ...
    double getTran(int x, int y, int direction) {
        if (direction == NORTH) {
    ...
    int automaticAction() {
        ...
        if (knownPosition) {
            ...
            //double stay = Vs[startX][startY];
            double up = Vs[startX][startY-1];
            double down = Vs[startX][startY+1];
            double left = Vs[startX-1][startY];
            double right = Vs[startX+1][startY];

            double arr[] = {up, down, left, right};
            Arrays.sort(arr);
            //System.out.println("arr[4] = " + arr[4]);
            //System.out.println("up: " + up + " down: " + down + " left: " + left + " right: " + right + " stay: " + stay);
            if (arr[3] == up) {
                startY -= 1;
                return NORTH;
            } else if (arr[3] == down) {
    ...
    double findMax(int x, int y) {
        ...
        double up = Vs[x][y-1] * moveProb + (Vs[x][y+1] + Vs[x-1][y] + Vs[x+1][y]) * otherProb;
        double down = Vs[x][y+1] * moveProb + (Vs[x][y-1] + Vs[x-1][y] + Vs[x+1][y]) * otherProb;
        double right = Vs[x+1][y] * moveProb + (Vs[x-1][y] + Vs[x][y-1] + Vs[x][y+1]) * otherProb;
        double left = Vs[x-1][y] * moveProb + (Vs[x+1][y] + Vs[x][y-1] + Vs[x][y+1]) * otherProb;

        double arr[] = {up, down, right, left};
        Arrays.sort(arr);
            ...
            for (int y = 1; y < mundo.height - 1; y++) {
                for (int x = 1; x < mundo.width - 1; x++) {
                    ...
                    if (mundo.grid[x][y] == 0) {
                        // open
                        reward = 1;
                        Vs[x][y] = reward + gamma * findMax(x, y);
                        if (Math.abs(Vs[x][y] - oldV) < epsilon) {
                            converged = true;
                        }
                    }

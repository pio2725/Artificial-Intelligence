...
class Sudoku():
    ...
    def forward_check(self, row, column, value, mode='remove'):
        ...
        if mode == 'remove':
            ...
            for i in range(0, 9):
                if self.cells[row][i].value is None:
                    if not self.cells[row][i].remove_value(value):
                        return False
                if self.cells[i][column].value is None:
                    if not self.cells[i][column].remove_value(value):
                        return False
...
def mrv(puzzle, unassigned):
    ...
    for r, c in unassigned:
        if len(puzzle.cells[r][c].domain) < minimum:
            minimum = len(puzzle.cells[r][c].domain)
    for r, c in unassigned:
        if len(puzzle.cells[r][c].domain) == minimum:
            result.append((r, c))
...
def count_constraints(puzzle, row, column):
    ...
    for i in range(0, 9):
        if puzzle.cells[row][i].value is None:
            count = count + 1
        if puzzle.cells[i][column].value is None:
            count = count + 1
        r, c = Sudoku().get_row_column(grid, i)
...
def order_values(puzzle, row, column):
    ...
    # TASK 5 CODE HERE
    s = sorted(domain, key=lambda x: puzzle.forward_check(row, column, x, 'count'))
...
def backtracking_search(puzzle):
    ...
    values = order_values(puzzle, row, col)
    ...
    for value in values:
        ...
        new_puzzle.cells[row][col].assign_value(value)
        ...
        if not new_puzzle.forward_check(row, col, value, 'remove'):
            continue
